...
// import { Cart, CarItem } from "../../type/cartType";
// import {
//   sumCartItemMrpPrice,
...
//         (state, action: PayloadAction<CarItem>) => {
//           if (state.cart) {
...
>(
  ...
  async ({ jwt, cartItemId, cartItem }, { rejectWithValue }) => {
    try {
      ...
      console.log("cart Item updated success ", response.data);
      return response.data;
    } catch (error: any) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to update cart item"
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<Cart>) => {
          ...
          // ✅ normalize both "cartItems" and "carItems"
          state.cart = {
            ...data,
            cartItems: data.cartItems || (data as any).carItems || [],
          };
          state.loading = false;
        }
      )
