...
export default function ProductTable() {
  ...
  React.useEffect(() => {
    ...
    console.log("seller Product-----", sellerProduct.products);
  }, []);
  return (
    <TableContainer component={Paper}>
      <Table sx={{ minWidth: 700 }} aria-label="customized table">
        ...
        {/* <TableBody>
          {sellerProduct?.products && Array.isArray(sellerProduct.products) ? (
            sellerProduct.products.map((item: any) => (
              <StyledTableRow key={item.id}>
                <StyledTableCell>{item.imageUrl}</StyledTableCell>
                <StyledTableCell align="right">{item.title}</StyledTableCell>
                <StyledTableCell align="right">{item.mrp}</StyledTableCell>
